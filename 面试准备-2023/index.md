...
3. 通用素材加载拦截器：跳转剪辑页之前，如果检测到了scheme中携带了素材，且未加载，则进入素材加载的页面，显示全屏loading，下载完成后进入剪辑页，当然下载失败不阻塞用户的流程
4. 异常处理逻辑：包括版权素材不可剪情况下的异常提示（无版权）、视频换链失败异常提示、
5. 封装通用的放号授权逻辑：原本就有一份放号逻辑，但是与业务耦合严重，无法直接复印，参考Glide的实现封装到一个空的Fragment中，对外提供统一的接口；在落地页，如剪辑页、拍摄页进行弹窗授权
6. 正确上报用户视频使用素材：尽量保证修改所影响的范围，梳理各个场景的逻辑，增补相应的字段即可；其中也发现了问题，当时是后台同学发现存在上报的字符串被截断了的case，后台同学排查发现数据入库存在字符串长度限制（6000），同时端上也找到了上报的素材存在超长的case（例如在剪辑工具下使用了各种素材），意味着端上上报需要按照产品定义的优先级去尽量上报素材，并且限制每种类型的素材的上报数量限制；
7. 版本兼容机制；低版本刷到高版本才支持的xx拍同款，需要提示用户升级；scheme中会携带min_version，Router中添加版本拦截器用于校验版本
...
##### 流程
camera输出到->SurfaceTexture

SurfaceTexture是渲染链提供的纹理texutre包装而成

opengl环境中创建一个texture id 用于提供给相机作为输出，而对于渲染链来说则是输入

SurfaceTexture可用的时候则会回调，回调则需要调用updateTexImage，将纹理图像转化为图像流中的最新帧

然后再输入给渲染链，渲染链中的各个节点依次进行处理，各节点输入一个纹理，输出一个纹理

预处理节点->light节点->暗角节点->合拍节点->高斯模糊节点

渲染链输出一个纹理，给到1. 屏幕的GlSurfaceView，用于预览 2. 如果点击了拍摄，则给到视频录制器进行视频录制

屏幕渲染是GlSurfaceView承载，提供一个Surface，opengl环境绑定Surface渲染

视频录制则是MediaCodec实现录制，提供一个Surface，opengl环境绑定Surface渲染
音频也是MediaCodec实现录制，
最后通过MediaMuxer进行封装
